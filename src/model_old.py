...
class JEPA_FeatureMask(nn.Module):
    def __init__(self, encoder, predictor, grid_size=7, context_ratio=0.5):
        super().__init__()
    ...
    def create_feature_masks(self, batch_size):
        ...
        for _ in range(batch_size):
            ...
            target_idx = indices[num_context:]
    ...
    def forward(self, x):
        ...
        context_mask = context_mask.to(x.device)  # [B,7,7]
        target_mask = target_mask.to(x.device)    # [B,7,7]

        # Encode full image
        with torch.no_grad():
            target_features = self.encoder(x)  # [B,512,7,7]

        # Apply context mask to features        
        # Mask out target features not in context
        # Build a mask for context to zero out masked positions
        cm = context_mask.unsqueeze(1).float()  # [B,1,7,7]
        context_features = target_features * cm

        # Predictor tries to reconstruct masked regions
        pred_features = self.predictor(context_features)

        # Compute loss only over masked (target) regions
        tm = target_mask.unsqueeze(1).float()  # [B,1,7,7]
        loss = F.mse_loss(pred_features * tm, target_features * tm)

        return loss, pred_features, target_features, context_mask, target_mask
